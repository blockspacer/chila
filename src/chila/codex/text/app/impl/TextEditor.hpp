...
MY_NSP_START
{
    class TextEditor final: public connection::TextEditor::CProvider
    {
        public:
            ...
            void MOD_ACTION_SIG(tabAdded);

        private:
            QQmlApplicationEngine &qEngine;
            QQmlComponent textItemsComp;
            QObject &textItems;

            struct
            {
                clQt::SigConnUPtr textChanged;
            } qtSigConns;
...
//
            ...
            using DataMapIt = std::map<const QObject*, DataMap::iterator>;
//
...
//
            ...
            Data *getCurrData();
            Data &getData(const QObject &textArea);
            Data *getDataPtr(const FileIdSCPtr &fileId);
            Data *getDataPtr(const QObject &textArea);
...
//
            template <typename EventExecuter>
            void saveEditorContents(Data &data, const EventExecuter &eventExecuter);
//
//            template <typename EventExecuter>
...
//
//            void createAction(
//                ccCommon::ActionPathDataList aList,
