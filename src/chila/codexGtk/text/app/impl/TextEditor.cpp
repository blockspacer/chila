...
MY_NSP_START
{
    ...
    void TextEditor::registerActions(const EventExecuter &eventExecuter)
    {
        for (const auto &actionData : modActions.dataList)
            execute_event_tn(registerAction)(actionData.aList, actionData.action);
    }

    void TextEditor::MOD_ACTION_SIG(launcher_start)
    {
        registerActions(eventExecuter);
    ...
    TextEditor::TextEditor(module_args_ex) :
        ...
        postFun(postFun)
    {
        ccCommon::ActionDataList editAList = { { "edit", "Edit" } };

        modActions.createAction(editAList, "copy",         "Copy",             "Copies the text.",             Gtk::Stock::COPY,   { 'c', Gdk::CONTROL_MASK });
        modActions.createAction(editAList, "paste",        "Paste",            "Pastes the text.",             Gtk::Stock::PASTE,  { 'v', Gdk::CONTROL_MASK });
        modActions.createAction(editAList, "cut",          "Cut",              "Cuts the text.",               Gtk::Stock::CUT,    { 'x', Gdk::CONTROL_MASK });
        modActions.createAction(editAList, "undo",         "Undo",             "Undoes the last operation.",   Gtk::Stock::UNDO,   { 'z', Gdk::CONTROL_MASK });
        modActions.createAction(editAList, "redo",         "Redo",             "Redoes the last operator.",    Gtk::Stock::REDO,   { 'y', Gdk::CONTROL_MASK });
        modActions.createSeparator(editAList),
        modActions.createAction(editAList, "findReplace",  "Find & Replace",   "Find and replace text.",       Gtk::Stock::REDO,   { 'f', Gdk::CONTROL_MASK });
    ...
    {
        openEditorPrv(createFileIdFun(), "(new)", "", eventExecuter);
    }
    ...
    void removeTrailingWSpace(const Glib::RefPtr<Gtk::TextBuffer> &buffer)
    {
        auto rangeLeft = boost::make_iterator_range(buffer->begin(), buffer->end());
        ...
        while (boost::distance(rangeLeft))
        {
            ...
            {
                auto it = buffer->erase(spaceRange.begin(), spaceRange.end());
                rangeLeft = boost::make_iterator_range(it, buffer->end());
            }
            else
            {
                rangeLeft = boost::make_iterator_range(spaceRange.end(), buffer->end());
            }
   ...
   struct FindTextIterNValNL
    ...
    {
        auto &scrolled = clGtkmm::makeManagedRef<Gtk::ScrolledWindow>();

        auto ret = dataMap.insert({fileId, { fileId, parserId }});
    ...
    void TextEditor::closingEditorPrv(Data &data, const EventExecuter &eventExecuter)
    {
        if (!data.isOriginalContents())
        {
            ...
            {
                postFun("dialog->signal_response()", [this, dialog, response, &data, eventExecuter]
                {
                    switch (response)
                    {
    ...
    template <typename EventExecuter>
    void TextEditor::saveEditorContents(const Data &data, const EventExecuter &eventExecuter)
    {
        removeTrailingWSpace(data.textView.get_buffer());

        char buff[BUFFER_SIZE];
        auto end = data.getContents(buff, sizeof(buff));
        execute_event_tn(saveEditorContents_current)(data.id, BufferRange(buff, end));
    }
    ...
    void TextEditor::MOD_ACTION_SIG(saveEditorContents_as)
    {
        ...
        execute_event(saveEditorContents_as)(data.id, fileName, BufferRange(buff, end));
    }
    ...
    void TextEditor::MOD_ACTION_SIG(savingDone)
    {
        ...
        postFun("event: savingDone", [this, eventExecuter]
        {
