...
//../../../../src/chila/codexGtk/base/app/impl/macros.fgen.py
#ifndef MY_NSP_START\

    #define MY_NSP_START \
        CHILA_LIB_MISC__DEF_NAMESPACE_VAR(chila,codexGtk,base,app,impl)

    #define MY_NSP_END \
        CHILA_LIB_MISC__CLOSE_DEF_NAMESPACE_VAR(chila,codexGtk,base,app,impl)
    ...
    #define my_get_object(Type, name) \
        objects.name = Glib::RefPtr<Gtk::Type>::cast_dynamic(builder->get_object(BOOST_PP_STRINGIZE(name))); \
        my_assert(objects.name)
    ...
    #define module_pass_args_ex \
        CHILA_CONNECTIONTOOLS_LIB_OTHER__MODULE_PASS_ARGS_EX(CHILA_CODEX_BASE_APP_IMPL_CONNECTION, MODULE_NAME)
