...
#ifndef CHILA_LIB_TINYXMLUTILS__UTIL_HPP
#define CHILA_LIB_TINYXMLUTILS__UTIL_HPP
...
DEF_NAMESPACE(3, (chila,lib,tinyXMLUtils))
{
    ...
    inline std::string getAttribute(const TiXmlElement &element, const char *name)
    {
        const char *v = element.Attribute(name);
        if (!v)
            BOOST_THROW_EXCEPTION(MissingNode()
                                     << chila::lib::misc::ExceptionInfo::Row(element.Row())
                                     << chila::lib::misc::ExceptionInfo::Column(element.Column())
                                     << chila::lib::misc::ExceptionInfo::XPath(getPath(element) + "/@" + name));
        return v;
    }

    const TiXmlElement &getElement(const TiXmlElement &element, const char *name);
